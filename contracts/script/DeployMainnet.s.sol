...
/// @notice Robinhood Chain mainnet (4663). Registers each stock with its Chainlink feed
///         AND the Uniswap v4 pool used to buy it at settlement.
///
...
contract DeployMainnet is Script {
    ...
    // token, Chainlink feed, and the v4 pool tier discovered on-chain
    ...
    function run() external {
        ...
        PackSale sale = new PackSale(IERC20(USDG), IJackpotVault(address(vault)), IPoolManager(POOL_MANAGER));
        vault.setPackSale(address(sale));

        S[20] memory stocks = _stocks();
        for (uint256 i = 0; i < stocks.length; i++) {
            sale.setStock(stocks[i].token, stocks[i].feed, stocks[i].fee, stocks[i].tickSpacing);
        }
        ...
        sale.addPack(10e6, all); // Starter $10

        uint256[8] memory blue = [uint256(1), 3, 4, 5, 6, 0, 2, 14]; // AAPL MSFT AMZN GOOGL META NVDA TSLA TSM
        address[] memory bluechip = new address[](blue.length);
        for (uint256 i = 0; i < blue.length; i++) {
            bluechip[i] = stocks[blue[i]].token;
        }
        sale.addPack(25e6, bluechip); // Blue Chip $25

        uint256[8] memory ai = [uint256(0), 7, 14, 16, 19, 15, 8, 3]; // NVDA AMD TSM MU ASML INTC PLTR MSFT
        address[] memory aiPack = new address[](ai.length);
        for (uint256 i = 0; i < ai.length; i++) {
            aiPack[i] = stocks[ai[i]].token;
        }
        sale.addPack(50e6, aiPack); // AI $50

        console.log("Vault    ", address(vault));
        console.log("PackSale ", address(sale));
        console.log("stocks registered:", stocks.length);
